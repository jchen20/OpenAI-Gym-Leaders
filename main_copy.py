...
class RLEnvPlayer(Gen8EnvSinglePlayer):
    def embed_battle(self, battle):
        ...
        team_one_hot_types = one_hot(battle.active_pokemon.types,18)
    
        opponent_one_hot_types = one_hot(battle.opponent_active_pokemon.types,18)
        
        moves_categories = moves_categories - 1
        category_matrix = np.zeros((4,3))
        category_matrix[np.arange(np.size(moves_categories)),moves_categories] = 1
